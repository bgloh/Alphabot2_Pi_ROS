...
while True:
    ...
    canny = cv2.Canny(cv2.cvtColor(img_resize, cv2.COLOR_BGR2GRAY), 10, 90)
    crop_canny = canny[int(height/2):,:]
    crop_canny_resize = cv2.resize(crop_canny, (width, height))
    crop_img = img_resize[int(height/2):,:,:]
    crop_resize = cv2.resize(crop_img, (width, height))
